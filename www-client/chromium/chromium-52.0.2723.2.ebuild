...
IUSE="cups gn gnome gnome-keyring gtk3 +hangouts hidpi hotwording kerberos neon pic +proprietary-codecs pulseaudio selinux shared-libs +system-ffmpeg +tcmalloc widevine"
...
src_configure() {
	...
	# This is unsupported by upstream since they do static builds by default,
	# but on memory constrained machines may help some during build (and for
	# debugging purposes.
...
src_install() {
	...
	# When doing a shared_library build, we have a lot of so files in
	# out/Release/lib that Chrome is built against.
	if use shared-libs; then
		insinto "${CHROMIUM_HOME}"
		doins out/Release/lib/*.so
	fi
